...
class BonusMazeworldProblem:

    ## you write the constructor, and whatever methods your astar function needs

    def __init__(self, maze, goal_locations):
        self.maze = maze
        self.start_state = tuple(self.maze.get_robotloc())
    ...
    def get_successors(self, state):
        successors = [state]
        for i in range(int(len(self.maze.robotloc)/2)):
            sub_successors = successors
            successors = []
            while(sub_successors != []):
                successors += self.build_successors(tuple([i]+list(sub_successors.pop())))

        return successors

    def build_successors(self,state):
    ...
    def get_cost(self, state1, state2):
        step = 0
        for i in range(int(len(self.maze.robotloc)/2)):
            step += abs(state1[i]-state2[i]) + abs(state1[i+1]-state2[i+1])
        return step*self.fuel

    def goal_test(self,state):
        for i in range(int(len(self.maze.robotloc))):
            if self.goal_locations[i] != state[i]:
                return False
        return True
    ...
    def manhattan_heuristic(self, state):
        ...
        for i in range(num_robot):
            distance += abs(state[2*i]-self.goal_locations[2*i]) + abs(state[2*i+1]-self.goal_locations[2*i+1])
        return distance
    ...
    def wavefront_bfs_heuristic(self, state):
        ...
        for robot_index in range(len(self.cost_dict)):
            cost += self.cost_dict[robot_index][(state[2*robot_index],state[2*robot_index+1])]
        return cost
